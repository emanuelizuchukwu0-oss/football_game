...
class Referee {
    ...
    update(ball, game) {
        ...
        if (ballDistance > 100) {
            this.targetX = ball.x + (Math.random() - 0.5) * 80;
            this.targetY = ball.y + (Math.random() - 0.5) * 60;
            this.isRunning = true;
        } else if (ballDistance < 60) {
            this.targetX = ball.x + (Math.random() - 0.5) * 120;
            this.targetY = ball.y + (Math.random() - 0.5) * 100;
            this.isRunning = true;
        } else {
            this.isRunning = false;
        }
    ...
    draw(ctx) {
        ...
        ctx.fillStyle = this.color;
        const armSwing = this.isRunning ? Math.sin(this.animationFrame * 0.3) * 15 : 0;
        ...
        const legSwing = this.isRunning ? Math.sin(this.animationFrame * 0.3) * 20 : 0;
        
        ctx.save();
        ctx.rotate(legSwing * Math.PI / 180);
        ctx.fillRect(-3, 5, 3, 20);
        ctx.restore();
...
// FIXED FootballGame class with proper timing
class FootballGame {
    constructor() {
        ...
        // FIXED: Use only gameTime for timing
    ...
    update() {
        ...
        // FIXED: Update game time properly
    ...
    setupPlayers() {
        ...
        // Reset user control - CHANGED: No initial user control
        this.userControlledPlayer = null;
        ...
        homePositions.forEach((position, index) => {
            ...
            // CHANGED: User only controls when they have the ball
            let isUserControlled = false;
            let isCurrentUserControlled = false;
            
            // Only set as user controlled if this is the user's team in player-vs-computer mode
            if (this.gameMode === 'player-vs-computer' && this.userControlledTeam === 'home') {
                isUserControlled = true;
                // Start with no control until they get the ball
                isCurrentUserControlled = false;
            }
    ...
    checkBallPossession() {
        if (Math.abs(this.ball.speedX) > 5 || Math.abs(this.ball.speedY) > 5) {
            if (this.ball.possessedBy) {
                ...
                // CHANGED: Remove user control when losing possession
        ...
        if (!this.ball.possessedBy) {
            ...
            if (closestPlayer && closestDistance < 35) {
                ...
                // CHANGED: Only give user control if it's their team and they have the ball
                if ((closestPlayer.isHomeTeam && this.userControlledTeam === 'home') ||
                    (!closestPlayer.isHomeTeam && this.userControlledTeam === 'away')) {
    ...
    switchUserControl(newPlayer) {
        ...
        // Give control to new player only if they have the ball
    ...
    userShoot() {
        // CHANGED: Only allow shooting if user has control and the ball
        if (this.userControlledPlayer && this.userControlledPlayer.hasBall && this.ball.possessedBy === this.userControlledPlayer) {
            ...
            // CHANGED: Remove user control after shooting
            this.userControlledPlayer.isCurrentUserControlled = false;
            this.userControlledPlayer = null;
        }
    }
    ...
    userPass() {
        // CHANGED: Only allow passing if user has control and the ball
        if (this.userControlledPlayer && this.userControlledPlayer.hasBall && this.ball.possessedBy === this.userControlledPlayer) {
            ...
            if (bestTeammate) {
                ...
                // CHANGED: Remove user control after passing
                this.userControlledPlayer.isCurrentUserControlled = false;
                this.userControlledPlayer = null;
            }
        }
    ...
    userTackle() {
        // CHANGED: Only allow tackling if user doesn't have the ball
    ...
    handleGoal(isHomeTeam) {
        this.lastGoalTime = this.gameTime;
        this.celebrationTime = 180;
    ...
    resetPlay() {
        ...
        this.players.forEach(player => {
            ...
            // CHANGED: Reset user control on reset
            if (player.isCurrentUserControlled) {
                player.isCurrentUserControlled = false;
            }
        });
        
        // CHANGED: Reset user controlled player
        this.userControlledPlayer = null;
    ...
    render() {
        ...
        if (this.celebrationTime > 0) {
            this.ctx.fillStyle = `rgba(255, 215, 0, ${0.3 + Math.sin(this.gameTime * 10) * 0.2})`;
    ...
    updateUI() {
        ...
        // FIXED: Timer now uses gameTime consistently
...
// ENHANCED AnimatedPlayer class with more realistic graphics and improved AI
class AnimatedPlayer {
    ...
    calculateDerivedAttributes() {
        this.baseSpeed = 2 + (this.attributes.pace / 100) * 3;
        this.acceleration = 0.15 + (this.attributes.pace / 100) * 0.15;
        this.shooting = 0.5 + (this.attributes.shooting / 100) * 0.5;
        this.passing = 0.5 + (this.attributes.passing / 100) * 0.5;
        this.control = 0.5 + (this.attributes.dribbling / 100) * 0.5;
        this.tacklingSkill = 0.3 + (this.attributes.defending / 100) * 0.7;
        this.aggression = 0.3 + (this.attributes.physical / 100) * 0.7;
    }
    ...
    // FIXED: Complete AI implementation
    updateAI(ball, game) {
        this.actionTimer--;
        ...
        if (this.actionTimer <= 0) {
            this.makeAIDecision(ball, game, ballDistance);
            this.actionTimer = 30 + Math.random() * 40;
        }

        // Basic AI behavior based on ball possession
        if (ball.possessedBy) {
            if (ball.possessedBy.isHomeTeam === this.isHomeTeam) {
                this.supportTeammate(ball.possessedBy, ball, game);
            } else {
                this.defendAgainstOpponent(ball, ballDistance, game);
            }
        } else {
            if (this.shouldGoForLooseBall(ball)) {
                this.targetX = ball.x;
                this.targetY = ball.y;
            } else {
    ...
    makeAIDecision(ball, game, distance) {
        ...
        if (this.hasBall && ball.possessedBy === this) {
            ...
            // ENHANCED: Better AI shooting logic
            ...
            // ENHANCED: AI can now dribble and make runs
            if (Math.random() < this.attributes.dribbling * 0.03) {
    ...
    attemptDribble(ball, game) {
        ...
        // Change direction slightly to simulate dribbling
        const dribbleAngle = Math.atan2(this.speedY, this.speedX) + (Math.random() - 0.5) * 0.5;
        const dribblePower = 2 + Math.random() * 2;
        
        this.speedX += Math.cos(dribbleAngle) * dribblePower * 0.1;
        this.speedY += Math.sin(dribbleAngle) * dribblePower * 0.1;
    ...
    attemptShot(ball, game) {
        ...
        // ENHANCED: More realistic shooting based on attributes
        const basePower = 15 + (distanceToGoal / 50);
        const powerMultiplier = 0.7 + this.shooting * 0.6;
        const accuracy = this.shooting * 0.5 + 0.3;
        
        const randomOffset = (1 - accuracy) * 100;
        const targetX = goalX;
        ...
        const power = Math.min(30, basePower * powerMultiplier);
            ...
            game.players.forEach(player => {
                if (player !== this && player.isHomeTeam === this.isHomeTeam) {
                    ...
                    if (distance < 300 && distance > 50) {
                        ...
                        // ENHANCED: Better passing selection
                        score += (this.attributes.vision / 100) * 50;
    ...
    attemptTackle(ball, game) {
        ...
        if (distance < 40) {
            ...
            // ENHANCED: More realistic tackling based on attributes
            const successChance = this.tacklingSkill * 0.4 + (this.aggression * 0.3);
    ...
    // FIXED: Role determination methods
    setPositionBehavior() {
        switch(this.role) {
            case 'goalkeeper':
                this.aggression = 0.3;
    ...
    // ENHANCED: More realistic player drawing
    draw(ctx) {
        ctx.save();
        ctx.translate(this.x, this.y);
        ...
        // ENHANCED: More realistic body proportions
        ...
        for (let i = -20; i < 0; i += 8) {
            ctx.fillRect(-8, i, 16, 4);
        }

        // ENHANCED: More realistic head
        ctx.fillStyle = '#FFDBAC';
        ctx.beginPath();
        ...
        ctx.beginPath();
        ctx.arc(0, -30, 1, 0, Math.PI, false);
        ctx.stroke();

        // Arms with more realistic animation
        ctx.fillStyle = this.color;
        ...
        ctx.save();
        ctx.rotate(-armAngle * Math.PI / 180);
        ctx.fillRect(2, -20, 10, 4);
        ctx.restore();

        // Legs with more realistic running animation
        ctx.fillStyle = this.secondaryColor;
        ...
        if (this.playerData) {
            ...
            ctx.fillText(lastName.substring(0, 8), 0, 25);
        }
...
// The rest of your classes (TouchControls, KeyboardControls, FormationOverlay, TeamPlaysOverlay) remain the same
// [Include all the other classes exactly as you had them...]
